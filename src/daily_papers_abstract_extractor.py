...
def json_to_markdown(json_data):
    ...
    for article in json_data:
        ...
        paper_id = paper["id"]
        keywords = paper.get("ai_keywords", "None")
        github_repo = paper.get("githubRepo", "No repo")
        github_stars = paper.get("githubStars", -1)
        if "organization" in article:
            organization = article["organization"].get("fullname", "")
        else:
            organization = "No org extracted."

        hf_link = f"https://huggingface.co/papers/{paper_id}"
        arxiv_link = f"https://arxiv.org/pdf/{paper_id}"

        markdown_content += f"## {title}(organization: {organization})\n\n"
        markdown_content += f"[Hugging Face]({hf_link}) | [PDF]({arxiv_link}) | [Github Stars: {github_stars}]({github_repo})\nkeywords: {keywords}\n\n"
